...
def get_routes(request, form) -> dict:
    context = {'form': form}
    qs = Trains.objects.all().select_related('from_city', 'to_city')
    graph = get_graph(qs)
    ...
    if len(routes) == 1:
        ...
        times = list({r['total_time'] for r in routes})
        times = sorted(times)
        for time in times:
            sorted_routes.extend(route for route in routes if time == route['total_time'])
